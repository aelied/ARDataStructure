// ================================================================
//  ARAssessmentPanelBuilder.cs
//  Place in:  Assets/Editor/ARAssessmentPanelBuilder.cs
//
//  Unity menu:  Tools → Build AR Assessment Panels
//  Adds the three assessment panels (Intro, Task, Results)
//  inside whichever Canvas is currently selected,
//  OR inside "LessonGuideCanvas" if one exists.
// ================================================================
#if UNITY_EDITOR
using UnityEngine;
using UnityEditor;
...
public class ARAssessmentPanelBuilder : EditorWindow
{
    // ── Colour palette ────────────────────────────────────────────
    static Color DarkBg       = new Color(0.06f, 0.07f, 0.16f, 0.97f);
    static Color CardBg       = new Color(0.10f, 0.12f, 0.30f, 1.00f);
    ...
    static Color TextMuted    = new Color(0.55f, 0.57f, 0.75f, 1.00f);
    static Color BarBg        = new Color(1f, 1f, 1f, 0.10f);

    [MenuItem("Tools/Build AR Assessment Panels")]
    public static void Build()
    {
        // Prefer the selected canvas; fall back to LessonGuideCanvas; then any canvas
        Canvas canvas = null;
        ...
        if (canvas == null)
        {
            Debug.LogError("[ARAssessmentPanelBuilder] No Canvas found in scene.");
            return;
        }

        BuildAssessmentPanels(canvas.transform);
        Debug.Log($"✅ AR Assessment panels built inside '{canvas.name}'. " +
    ...
    {
        // Root wrapper — disabled by default (assessment activates it)
        GameObject root = MakeRect("AssessmentRoot", canvasRoot);
        Stretch(root);
        root.SetActive(false);   // activated by ARArrayLessonAssessment at runtime
        ...
        Undo.RegisterCreatedObjectUndo(root, "Build AR Assessment Panels");
        Debug.Log("Hierarchy created. Now assign fields in ARArrayLessonAssessment component.");
    }
    ...
    // ─────────────────────────────────────────────────────────────
        static void BuildIntroPanel(Transform parent)
        {
            ...
            // Title
            GameObject title = MakeTMP("IntroTitleText", card.transform, "Lesson Assessment", 62, TextWhite, FontStyles.Bold);
            LE(title, 0, 80);
            ...
            GameObject startLabel = MakeTMP("StartLabel", startBtn.transform, "START ASSESSMENT  →", 44, TextWhite, FontStyles.Bold);
            Stretch(startLabel);
    ...
    static void BuildTaskPanel(Transform parent)
    {
        ...
        Stretch(panel);
        AddImage(panel, new Color(0, 0, 0, 0));   // transparent background
        ...
        AddImage(topBar, new Color(0.06f, 0.07f, 0.16f, 0.95f));
        ...
        fillImg.fillAmount = 0.25f;   // default — set at runtime

        HorizLayout(topBar, 0, TextAnchor.MiddleCenter, 40, 40, 22, 30);

        // Progress counter ("Task 1 / 4")
        GameObject counter = MakeTMP("TaskProgressText", topBar.transform, "Task 1 / 4", 36, TextMuted, FontStyles.Normal);
        LE(counter, 1, 0);

        // Timer
        GameObject timer = MakeTMP("TaskTimerText", topBar.transform, "⏱ 60s", 36, AccentYellow, FontStyles.Bold);
        LE(timer, 0, 80);
        timer.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Right;

        // ── Task card (anchored to bottom-left) ───────────────────
        GameObject card = MakeRect("TaskCard", panel.transform);
        SetAnchors(card, 0f, 0f, 1f, 0f);
        card.GetComponent<RectTransform>().sizeDelta        = new Vector2(0, 560);
        ...
        AddImage(card, new Color(0.06f, 0.07f, 0.18f, 0.97f));
        ...
        GameObject instr = MakeTMP("TaskInstructionText", card.transform,
            "Instruction goes here…", 36, TextMuted, FontStyles.Normal);
        LE(instr, 1, 0);
        var instrTmp = instr.GetComponent<TextMeshProUGUI>();
        instrTmp.alignment        = TextAlignmentOptions.Center;
        instrTmp.enableWordWrapping = true;

        // ── Feedback banner (hidden normally) ─────────────────────
    ...
    static void BuildResultsPanel(Transform parent)
    {
        ...
        // Grade ring image (use Filled circle image in Inspector)
        ...
        GameObject bkText = MakeTMP("ResultsBreakdownText", bkCard.transform,
            "✅ Task 1:  25/25\n❌ Task 2:  10/25", 36, TextMuted, FontStyles.Normal);
        ...
        GameObject tipIcon = MakeTMP("TipIcon", tipCard.transform, "💡", 50, TextWhite, FontStyles.Normal);
        SetAnchorsPoint(tipIcon, 0.04f, 0.5f);
        SetSize(tipIcon, 70, 70);

        GameObject tipText = MakeTMP("ResultsTipText", tipCard.transform,
            "Review: arrays use O(1) access via direct address.", 34, TextMuted, FontStyles.Normal);
        ...
        GameObject retryLabel = MakeTMP("RetryLabel", retryBtn.transform, "↩  Retry", 40, TextWhite, FontStyles.Bold);
        Stretch(retryLabel);
        ...
        GameObject returnLabel = MakeTMP("ReturnLabel", returnBtn.transform, "✓  Return to App", 40, TextWhite, FontStyles.Bold);
        Stretch(returnLabel);
        returnLabel.GetComponent<TextMeshProUGUI>().alignment = TextAlignmentOptions.Center;

        // Bottom padding
        GameObject pad = MakeRect("BottomPad", content.transform);
        LE(pad, 0, 60);
    }
